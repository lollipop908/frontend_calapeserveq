...
const ReportsPanel = ({ departments }) => {
  ...
  const { data: allQueuesData, loading: allLoading } = useQuery(
    GET_ALL_QUEUES_DEPARTMENT
  );
  const { data: deptQueuesData, loading: deptLoading } = useQuery(
    GET_QUEUES_BY_DEPARTMENT,
    {
  ...
  const processQueueData = () => {
    ...
    if (timeRange === "day") {
      ...
      const sortedData = Object.entries(yearlyCounts).sort(([a], [b]) => a - b);
  ...
  const avgValue =
    chartData.length > 0 ? Math.round(totalQueues / chartData.length) : 0;
  ...
  const downloadCSV = () => {
    ...
    a.href = url;
    a.download = `queue-report-${timeRange}-${
      new Date().toISOString().split("T")[0]
    }.csv`;
    a.click();
  };
          ...
          <div className="reports-title">
            ...
            <div>
              ...
              <p className="reports-subtitle">
                Real-time queue performance insights
              </p>
            </div>
          </div>
        </div>
